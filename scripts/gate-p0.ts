...
import { SessionManager } from '../apps/desktop/src/sessions/SessionManager.js';
import { CaptureSupervisor } from '../apps/desktop/src/capture/CaptureSupervisor.js';
import { ElectronBrowserAdapter } from '../packages/browser-adapter/src/electron/ElectronBrowserAdapter.js';
import { StreamAssemblyLedger } from '../packages/capture-core/src/assembly/StreamAssemblyLedger.js';
import { ProtocolCatalog } from '../packages/protocol-catalog/src/catalog/ProtocolCatalog.js';
import { DriftDetector } from '../packages/protocol-catalog/src/drift/DriftDetector.js';
import { Sanitizer } from '../packages/security/src/sanitizer/Sanitizer.js';

interface GateResult {
  gate: 'p0';
  timestamp: string;
  runtime_versions: { node: string; platform: string; arch: string; electron?: string };
  checks: Array<{ name: string; passed: boolean; details: string; measured?: unknown }>;
  passed: boolean;
...
async function main() {
  ...
  const sessionManager = new SessionManager('/tmp/test-partitions');
  const supervisor = new CaptureSupervisor(sessionManager);
  ...
  try {
    ...
    if (!attachedBefore) falsifiers.push('Attach did not occur before navigate');
  } catch (e) {
    checks.push({ name: 'attach_before_navigate', passed: false, details: (e as Error).message });
    falsifiers.push(`attach_before_navigate exception: ${(e as Error).message}`);
  }
  ...
  try {
    ...
    if (!allPresent) falsifiers.push('Missing target class in coverage');
  } catch (e) {
    checks.push({ name: 'target_coverage', passed: false, details: (e as Error).message });
  }
  ...
  try {
    ...
    checks.push({ name: 'renderer_loss_recovery', passed: false, details: (e as Error).message });
  }
  ...
  try {
    ...
    if (!matches) falsifiers.push('UTF-8 boundary reassembly failed');
  } catch (e) {
    checks.push({ name: 'utf8_boundary_split', passed: false, details: (e as Error).message });
  }
  ...
  try {
    ...
    checks.push({ name: 'stream_variants', passed: false, details: (e as Error).message });
  }
  ...
  try {
    ...
    if (!isolation.isolated) falsifiers.push('Account isolation failed: ' + isolation.details);
  } catch (e) {
    checks.push({ name: 'multi_account_isolation', passed: false, details: (e as Error).message });
  }
  ...
  try {
    ...
    checks.push({ name: 'protocol_catalog_rsc', passed: false, details: (e as Error).message });
  }
  ...
  try {
    ...
    const sanitized = Sanitizer.sanitizeJson(payload);
    ...
    if (leaked) falsifiers.push('Secret leaked through sanitizer');
  } catch (e) {
    checks.push({ name: 'sanitization_fail_closed', passed: false, details: (e as Error).message });
  }
  ...
  try {
    ...
    checks.push({ name: 'drift_detection', passed: false, details: (e as Error).message });
  }
  ...
  try {
    ...
    const r1 = q.enqueue({ id: '1', accountId: 'a', payload: { x: 1 } as any, enqueuedAt: Date.now(), sizeBytes: 10 });
    const r2 = q.enqueue({ id: '2', accountId: 'a', payload: { x: 2 } as any, enqueuedAt: Date.now(), sizeBytes: 10 });
    const r3 = q.enqueue({ id: '3', accountId: 'a', payload: { x: 3 } as any, enqueuedAt: Date.now(), sizeBytes: 10 }); // should overflow
    checks.push({
    ...
    if (r3.accepted) falsifiers.push('Bounded queue did not overflow as expected');
  } catch (e) {
    checks.push({ name: 'bounded_queue_overflow', passed: false, details: (e as Error).message });
  }
