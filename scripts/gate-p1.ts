...
import { ArchiveService } from '../packages/archive-service/src/service/ArchiveService.js';
import { DirectiveManager } from '../packages/security/src/directives/DirectiveManager.js';
import { ServiceRegistry } from '../packages/mcp-contract/src/serviceRegistry.js';
...
async function main() {
  ...
  const sessionManager = new SessionManager(path.join(dataDir, 'partitions'));
  ...
  try {
    ...
    for (const ev of events) supervisor.handleCdpEvent(ev as any);
    ...
    for (const ev of crashEvents) supervisor.handleCdpEvent(ev as any);
    ...
    serviceRegistry.register('archive_search', async (args: any) => {
      return { query: args.query, results: [{ turn_id: 'turn-1', text: 'Hello world complete stream', account_id: accountId }] };
    });
    serviceRegistry.register('archive_get_provenance', async (args: any) => {
    ...
    try {
      ...
      if (e.code === 'E_CONFIRM_REQUIRED' || e.code === 'E_UNSUPPORTED') mcpDeleteFailed = true;
    }
    checks.push({
    ...
    checks.push({
      ...
      passed: verify.ok,
      details: `Directive verification: ${verify.ok ? 'ok' : (verify as any).error}`,
    });
    ...
    checks.push({ name: 'exception', passed: false, details: (e as Error).message + '\n' + (e as Error).stack });
  } finally {
